...
docs/            # spec từng bước (orchestrator viết): SPEC-step1..32 + DESIGN-ai-native.md (PHẦN 0 = sai lệch ship-vs-design + nợ đã biết)
```
...
32. ✅ UX backlog pack — trọn 5 mục backlog session 2026-07-13 (goal user "hoàn thành all backlog ux"): **B1** đính kèm ảnh trong composer (POST messages + `attachments` ≤3 path `uploads/…` validate regex chống traversal, cột `messages.attachments`, note `[Đính kèm N ảnh…]` chỉ vào LLM context — cả history lẫn rebuild — DB giữ content gốc; FE 📎 + chip thumbnail + bubble ảnh); **B2** diff chip 🔧 `+N node · ~N param` trên bubble AI (live đếm patch-op + reload qua GET join `workflow_changes`, click = CTA mở split + fitView — trả nợ DESIGN PHẦN 0 mục 7); **B3** summary change giàu thông tin sinh tại flush-time (`sửa <key> của "<label>" (<type> <id>): <old> → <new>`, cắt 30 ký tự/giá trị; digest server `buildChangeDigest(changes, workflow?)` additive resolve label + `summarizeOps` kèm chi tiết khi 1 op); **B4** AI đặt tên workflow (cột `conversations.title_source` auto/user/ai, contract `{reply, ops, title?}`, `titleHint` additive trong system prompt, apply đổi cả `workflow.name`, SSE message kèm `title`, **fresh-check trước khi rename chống race đè tên user đặt giữa turn** — major finding từ review Opus); **B5** badge `[i2i]`/`[t2i]` trong ModelPicker (chỉ entry ảnh fal, không phá ARIA/keyboard). Smoke thật grok-4.5: title "Văn bản xin chào" source ai + diff `+2 node +1 nối` — spec: `docs/SPEC-step32.md`

33. ✅ **Pipeline "Video dài → Short + B-roll"** (yêu cầu user 2026-07-22) — spec: `docs/SPEC-step33.md`. Luồng: `input.file → video.transcribe → llm.selectMoments → flow.approveGate (⏸ chờ duyệt) → broll.generate → video.assembleShort → output.collect` (+ cạnh thứ 2 video gốc → assemble). **5 node mới + cơ chế gate:** **33a** `video.transcribe` (ffmpeg tách audio → `uploadToFal` fal storage REST `rest.fal.ai/storage/upload/initiate?storage_type=fal-cdn-v3` → wizper → segments; smoke thật PASS) + `CutPlanSchema`/`TranscriptSchema` trong `packages/shared`; **33b** `llm.selectMoments` (OpenRouter → CutPlan, dedupe id + cap maxMoments + clamp + re-validate); **33c** engine approval-gate — `GateRegistry` + node `flow.approveGate` await promise ngoài (run park trong inFlight, branch song song vẫn chạy) + NodeState `'awaiting'` + `ctx.awaitApproval` + **run-abort plumbing** (RunManager AbortController/run + `POST /api/runs/:id/stop`, trước đây KHÔNG có — plan-verify C1) + `POST /api/runs/:id/resume` (validate CutPlanSchema); **33d** `broll.generate` (loop fal.image t2i ×N, guard chặn model i2i — extract `findImageKind` khỏi fal.image) + `video.assembleShort` (ffmpeg cắt `-ss/-to` + chèn b-roll fullscreen still + anullsrc → 1 short 9:16, concat demuxer); **33e** FE panel `CutPlanReview` (sửa/duyệt/huỷ bản cắt, NodeState awaiting + badge, store awaitingGate) + sample `sample-video-to-short` + `mock-fal.ts` + e2e park→approve→short. Nợ đã biết: server restart lúc `awaiting` → run mồ côi, resume 409 (chấp nhận MVP local). Quy trình: Fable spec (củng cố sau Opus plan-verify) → Sonnet implement → **1 Opus review/bước** (user chốt 2026-07-22, giảm từ 2) → Sonnet fix → orchestrator verify + smoke.

Hiện trạng: **18 node types, catalog live ~1.240 model (576 ảnh + 319 video fal + 345 LLM) + 48 preset ⭐, 12 samples, 577 server + 34 shared + 425 web + 37 e2e tests.** Pipeline video→short xong (step 33). Việc sau này: tính năng mới theo yêu cầu user, vẫn theo luật orchestration ở trên. Nợ nhỏ đã biết: e2e app.spec test 19 (auto-split khi chọn conversation) thi thoảng flaky pass-on-retry (tiền sử từ step 23).
