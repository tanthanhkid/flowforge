...
3. **Sửa raw JSON qua JsonView và đổi tên workflow KHÔNG vào change log** (vi phạm cục bộ nguyên tắc "mọi thay đổi đều là PatchOp"): 2 luồng này vẫn đi đường PUT cũ — SPEC-step27.md §4 "hạn chế ghi nhận có chủ đích". AI vẫn thấy workflow mới nhất qua system prompt mỗi turn, chỉ digest/tab Lịch sử thiếu entry. **Cập nhật step 31 (SPEC-step31.md F8): PUT đổi nodes/edges nay ĐÃ ghi change row "Cập nhật thủ công (Save/JSON)" kèm snapshot (vá luôn lỗ hổng chuỗi snapshot làm revert mất dữ liệu); riêng rename vẫn không log (tránh noise).**
