...
```
input.file → video.transcribe → llm.selectMoments → flow.approveGate → broll.generate → video.assembleShort → output.collect
(mp4→video)   (fal wizper)        (OpenRouter)         ⏸ chờ duyệt        (fal.image ×N)   (ffmpeg cắt+ghép)
     └──────────────────────────────────────────────────────────────────────────────────────┘
        (cạnh thứ 2: video gốc cũng nối vào video.assembleShort.video)
```

Gate đặt **ngay trước** `broll.generate`: user duyệt/sửa bản cắt **trước khi** phát sinh chi phí gen ảnh + cắt.

**Node input video (plan-verify H1):** KHÔNG có `input.video` trong registry. Dùng **`input.file`** — nó map `mp4/mov/webm → MediaValue{kind:'video'}`, output port `file:{type:'any'}` nối được vào port `video` (schema cho `any → video`). Video gốc **fan ra 2 consumer**: `video.transcribe` VÀ `video.assembleShort` (assemble cần video gốc để cắt, KHÔNG phải output transcribe) — sample §33e phải vẽ cả 2 cạnh.
...
- **Lưu ý shape (plan-verify M1):** port `segments` là **mảng trần** `TranscriptSegment[]` (transcribe xuất 2 port riêng `text`+`segments`, KHÔNG có wrapper `{segments,text}` chạy trên edge). Node đã (đã ship) normalize cả mảng trần lẫn `{segments:[...]}` — không rework. `TranscriptSchema` wrapper chỉ dùng trong test.
...
### 33c — Engine approval-gate + run-abort (RỦI RO NHẤT)
...
**0. TIỀN ĐỀ — Run-abort plumbing (plan-verify C1, CRITICAL — phải làm TRƯỚC, gate cancel phụ thuộc):** Hiện KHÔNG có abort run: `runManager.start` không tạo `AbortController`, không truyền `signal` vào `engine.run`, `executor.ts:84` fallback signal không bao giờ fire; không có route stop; nút "■ Dừng" chỉ đóng SSE client-side. Thêm:
- `RunManager`: map `runId → AbortController`; `start()` tạo controller, truyền `{ signal: controller.signal }` vào `engine.run` (đã có `RunOptions.signal`). Xoá controller khi `run:state`.
- `runManager.stopRun(runId): boolean` → `controller.abort()`.
- `routes/runs.ts`: `POST /api/runs/:id/stop` → `stopRun`; 200 nếu active, 404/409 nếu không.
- Khi abort: `runSignal` fire → `gate.register`'s listener reject → node gate `finishNodeError` → cascade skip downstream → run `error`. (33e nút "Huỷ" gọi endpoint này.)
- Test: stop khi đang awaiting → gate reject, run error; stop khi đang chạy node thường → abort propagate.
...
4. **`executor.ts`**: trong `runNode`, khi build ctx, nếu engine có gate → set `awaitApproval: async (payload) => { upsertNode({nodeId, state:'awaiting', cacheHit:false, startedAt, outputs:{ pendingApproval: payload }}); emitNodeState(nodeId,'awaiting',{}); const v = await gate.register(runId,nodeId,payload,runSignal); return v; }`. Node awaiting KHÔNG thuộc `finished`, vẫn nằm trong `inFlight` → vòng `while(inFlight.size>0)` chờ tự nhiên, các branch song song vẫn chạy. Khi resolve, execute trả outputs thật → `finishNodeSuccess` ghi đè `pendingApproval`.
   - **Payload cho client live (plan-verify M2):** `emitNodeState` hiện chỉ nhận `{error?,cached?}` → client đang stream `node:state` KHÔNG thấy `pendingApproval` (chỉ client reconnect mới thấy qua `snapshot`→getRun đọc `outputs_json`). Widen `emitNodeState`/event `node:state` để mang kèm `pendingApproval` khi state=`awaiting` (thêm field optional, additive — RunManager forward nguyên). Nhờ vậy FE 33e không cần round-trip getRun riêng.
...
- **Guard extraction (plan-verify H2, làm TRƯỚC broll.generate):** `findImageKind`/`suggestI2iModels` đang private trong `fal.image.ts`. Extract ra helper dùng chung (vd `nodes/falImageKind.ts` hoặc trong `providers/fal.ts`), re-point `fal.image.ts` sang đó (giữ hành vi + test cũ xanh), rồi broll dùng lại. **Hướng guard NGƯỢC với fal.image:** broll KHÔNG có image input → phải chặn model **i2i** (nó cần ảnh mà broll không cấp được), gợi ý model t2i thay thế. (fal.image chặn "t2i + có ảnh"; broll chặn "i2i" — không copy nguyên điều kiện.)
- **`broll.generate`**: inputs `{ plan: {type:'json', required:true} }`, outputs `{ plan: {type:'json'} }`. params: `model` (fal image **t2i**, default rẻ vd `fal-ai/flux/schnell`), `imageSize` (default `portrait_16_9` / 1080×1920 hợp short), `skipEmptyPrompt` (default true). execute: lặp `plan.moments`, mỗi moment có `brollPrompt` → `runFalQueue` (t2i) → `downloadBinary` → `ctx.saveArtifact` → gán `moment.brollImage.path`. Tôn trọng abort (dừng vòng lặp). Log tiến độ `i/N`. Guard i2i như trên. **cacheable: true** (key theo plan + model).
...
- **Store/SSE:** xử lý `node:state` với `state:'awaiting'` → đọc `pendingApproval.plan` từ **chính event** (đã widen ở 33c M2; client reconnect lấy từ `snapshot`→getRun `outputs.pendingApproval`) → set `awaitingGate = { runId, nodeId, plan }`. Web `NodeState` union (`api/types.ts`) thêm `'awaiting'` + badge trong `NodeCard.tsx` (33c chỉ đụng BE, FE state là việc của 33e).
- **`CutPlanReview` panel:** hiện khi có `awaitingGate`. Liệt kê moments (start–end, title, reason, brollPrompt) — **sửa được** (title/start/end/brollPrompt, thêm/xoá moment). Nút **"Duyệt & cắt"** → `POST /api/runs/:id/resume { nodeId, output: {moments} }`, clear awaitingGate. Nút **"Huỷ"** → `POST /api/runs/:id/stop` (run-abort thêm ở 33c C1) → gate reject, run error. Đặt panel ở CanvasPane tab phải hoặc overlay ChatPane (chọn nơi ít phá layout nhất; badge nhắc khi ẩn).
